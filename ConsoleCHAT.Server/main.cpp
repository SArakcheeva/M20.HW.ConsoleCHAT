            ...
            if(vector_messageCS.size() < 1) {
                ...
                switch(id_cmd)
                {
                    ...
                    case 7: {/*Загрузить сообщения, адресованные заданному пользователю*/
                        ...
                        database::loadMessages(mess_user, messagesToUser);              //Заполнить вектор - сообщениями пользователю
                        ...
                        if (messagesToUser->empty()) {
                            ...
                            for (const auto& message : *messagesToUser) {
                                user_messages = message.getNameFrom() + ":" + message.getText() + "||";
                            }
                        }
                    ...
                    case 8: {/*Добавить в базу сообщение от одного пользователя другому*/
                        Message message(vector_messageCS[1], vector_messageCS[2], vector_messageCS[3]);	//Создать сообщение
